...
- 收藏状态与掌握状态解耦
- 同科目独立统计、错题本、收藏本
- 数学公式渲染支持：**Markdown + LaTeX(KaTeX)**
- 信号与系统第 1 章白皮书例题导入（当前已整理到 **例 1.1 ~ 例 1.10**）
...
当前重点完成了 **第 1 章：信号的函数表示与系统分析方法** 的例题整理，已导入：

- 例 1.1：由阶跃函数表达式快速画波形
- 例 1.2：导数、积分与符号函数对应波形
- 例 1.3：利用冲激抽样与阶跃性质求函数值
- 例 1.4：已知波形下的时域变换
- 例 1.5：两个已知波形相乘后的结果判断
- 例 1.6：由复合自变量波形反求原信号
- 例 1.7：由阶跃响应形式判断系统性质
- 例 1.8：由组合输入输出反推单位阶跃响应
- 例 1.9：由微分方程画系统方框图
- 例 1.10：由方框图反推微分方程
...
src/
  app/
    layout.tsx                 # 根布局、metadata、全局样式入口
    page.tsx                   # 首页 server entry
  components/
    dashboard/
      study-wall-dashboard.tsx # 主工作台界面
    ui/
...
3. **视频 / 笔记 / 纠错仍是占位入口**
4. **题目原图尚未嵌入卡片**
5. **白皮书导入仍是前端展示友好的整理版，不是完整数据库入库版**
6. **尚未完成 AI 批量导入流水线**
